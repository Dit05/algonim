...
export class Gif {
  ...
  */
  ...
  private emitLogicalScreenDescriptor(vec: ByteVector) {
    ...
    // TODO (don't forget to clamp)
    let packedField: number = 0

    // The background color is only meaningful if there's a global table.
    let hasGlobalTable: boolean = true
    packedField |= (hasGlobalTable ? 1 : 0) << 7

    // Number of bits per primary color available to the original image, minus 1.
    let colorBits: number = 1
    packedField |= colorBits << 5
    // Whether the global color table is sorted by decreasing importance.
    let globalTableSorted: boolean = false
    packedField |= (globalTableSorted ? 1 : 0) << 4
    // Number of colors = 2**(size + 1).
    let globalTableSize: number = 0
    packedField |= globalTableSize

    vec.addUint8(packedField)

    let backgroundColorIndex: number = 0
    vec.addUint8(backgroundColorIndex)
    ...
    if(hasGlobalTable) {
      this.emitGlobalColorTable(vec)
    }
  }

  private emitGlobalColorTable(vec: ByteVector) {
    // HACK
    for(let val of [0, 0, 0, 255, 255, 255]) {
      vec.addUint8(val)
    }
  }
